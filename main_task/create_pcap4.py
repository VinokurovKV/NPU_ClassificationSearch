...
packets = [
    # Вариант 4(каждому порту по 2 пакета, но у некоторых неправильные vlan, т.е. на двух портах будет по два пакета, а на трёх других - по одному):
    ...
    Ether(dst="12:33:44:55:66:77", src="aa:bb:cc:dd:ee:ff") / Dot1Q(vlan=20) /
        IP(src="192.168.2.1", dst="192.168.2.2") / UDP(sport=5678, dport=9012) /
            Raw(load=b"A"*73),
    Ether(dst="0A:BC:DF:E0:12:34", src="aa:bb:cc:dd:ee:ff") / Dot1Q(vlan=100) /
        IP(src="192.168.2.1", dst="192.168.2.2") / UDP(sport=6789, dport=1234) /
            Raw(load=b"A"*73),
    Ether(dst="0C:DE:F0:12:34:56", src="aa:bb:cc:dd:ee:ff") / Dot1Q(vlan=20) /
        IP(src="192.168.2.1", dst="192.168.2.2") / UDP(sport=7890, dport=2346) /
            Raw(load=b"A"*73),
    Ether(dst="0E:F1:23:45:67:89", src="aa:bb:cc:dd:ee:ff") / Dot1Q(vlan=300) /
        IP(src="192.168.2.1", dst="192.168.2.2") / UDP(sport=8901, dport=3456) /
            Raw(load=b"A"*73),
    Ether(dst="10:11:22:33:44:55", src="aa:bb:cc:dd:ee:ff") / Dot1Q(vlan=400) /
